...
def create_app() -> Flask:
    ...
    class ScoreFileResource(Resource):
        def post(self):
            ...
            default_essay_set = int(default_essay_set_raw)
            ...
            try:
                if filename.endswith(".csv"):
                    ...
                    rows = [{"essay": essay, "essay_set": default_essay_set} for essay in essays]
                else:
                ...
                for idx, (item, pred) in enumerate(zip(rows, scored), start=1):
                    ...
                    essay_set_value = pred["essay_set"]
                    score_value = pred["score"]
                    scaled_value = pred["scaled_score"]
                    result_item = {
                        "row": idx,
    ...
    class HistoryListResource(Resource):
        def get(self):
            ...
            if essay_set_value == "all":
                filter_value: int | str = "all"
            else:
                try:
                    filter_value = int(essay_set_value)
                except ValueError:
                    return {"error": "essay_set must be 'all' or an integer 1..8"}, 400
