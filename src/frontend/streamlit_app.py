...
def build_export_csv(submission_id: int) -> bytes:
    ...
    for idx, record in enumerate(records, start=1):
        ...
        essay_set_key = str(int(essay_set_raw))
    ...
    if submit:
        if not essay_text.strip():
            ...
            try:
                ...
                result = call_score_text(essay_text, int(essay_set_value))
                with col_right:
    ...
    with batch_right:
        if start_batch:
            if upload is None:
                ...
                try:
                    essay_set_value = selected_prompt[1]
                    result = call_score_file(upload, int(essay_set_value))
    ...
    if selected_id:
        ...
        if not items:
            ...
            with pager_wrap[1]:
                ...
                with pager_cols[0]:
                    if st.button("上一页", key="history_prev_btn", type="primary", disabled=st.session_state["history_page"] <= 0, use_container_width=True):
